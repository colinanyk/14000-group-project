...
public class Algorithm {//states class header
    ...
    final int range = 94; // this is used for the randomizer object to generate a random value only from 0-94, so values don't shift past the range of ASCII characters being used.
                          //using 32 to 126 ascii range, won't throw errors or weird characters;
                          
    public String algorithmize(CypherMessage cypherMessage) { //states algorithmize method, returns either a encoded/decoded string from two string inputs message and action
    
         int i; //used in for loop indexing
         int key = cypherMessage.getKey(); //parses key as integer from the user inputted string
         ...
         String action = cypherMessage.getAction(); // Declare and initialize action variable to the returned value of getAction()
        ...
        for (i=0; i < charArray.length ;i++) { //for loop runs from the 0 to end, adjusts end to length of message (charArray). increments i by 1 each time
            ...
            if ((charArray[i] + rand) > 126) { //for chars exceeding ascii range with rand value.
               charArray[i] = (char) ((charArray[i] + rand) - 95);
            } //the values of the characters exceeding 126 shift up the remaining amount from the bottom of the range
            else { //if the value don't exceed top of ascii range, they simply shift up by random generated value
                charArray[i] = (char) (charArray[i] + rand);
            }
        }
            ...
            System.out.println("Decoding Started..."); //lets user know encode has triggered
             ...
             for (i=0; i < charArray.length ;i++) {
                if ((charArray[i] - rand) < 32) { //for the chars that had previosuly exceeded ascii range with rand value.
                    charArray[i] = (char) ((charArray[i] + 95) - rand);
                } //the values shift back down from the top of the range using the remaining amount
                else {
